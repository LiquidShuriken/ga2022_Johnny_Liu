...
int main(int argc, char* argv[])
{
    ...
    // output message buffer, assume max character count is 256
    int buf_size = 256;
    char* msg = heap_alloc(heap, buf_size *sizeof(char), 8);
    char welcome[] = "Welcome to Zork!";
    strncpy_s(msg, strlen(welcome)+1, welcome, buf_size);
    // input command buffer, assume max character count is 256
    char* input = heap_alloc(heap, buf_size * sizeof(char), 8);
    ...
    while (!quit)
    {
        ...
        // show a simple window that we created ourselves.
        {
            ...
            igBegin("I guess this is kinda like Zork?", NULL, 0);
            igText(msg);

            if (igInputText("What do you do?", input, buf_size, input_text_flags, 0, NULL))
            {
                if (strncmp(input, "hello", strlen(input)) == 0)
                {
                ...
                {
                    char invalidcmd[] = "Unknown command.";
                    strncpy_s(msg, strlen(invalidcmd) + 1, invalidcmd, buf_size);
                }
                strcpy_s(input, 1, "");
            }
            igEnd();
