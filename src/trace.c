...
trace_t* trace_create(heap_t* heap, int event_capacity)
{
	...
	trace->event_buffer = calloc(trace->event_capacity * 80, sizeof(char));
...
void trace_duration_pop(trace_t* trace)
{
	...
	if (trace->capturing)
	{
		...
		strncat_s(trace->event_buffer, trace->event_capacity * 80, info, strlen(info));
	}
...
{
	snprintf(info, 80, "{\"name\":\"%s\",\"ph\":\"%c\",\"pid\":0,\"tid\":\"%d\",\"ts\":\"%d\"},\n",
